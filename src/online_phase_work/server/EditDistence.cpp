...
//1. 求取一个字符占据的字节数
size_t EditDistence::nBytesCode(const char ch) {
    if(ch & (1 << 7))
    {
        int nBytes = 1;
...
//2. 求取一个字符串的字符长度
std::size_t EditDistence::length(const std::string &str) {
    std::size_t ilen = 0;
...
//3. 中英文通用的最小编辑距离算法
int EditDistence::editDistance(const std::string & lhs, const std::string &rhs) {
    //计算最小编辑距离-包括处理中英文
